...
public class InMemoHedwigMessage implements HedwigMessageI{
     ...
     * @param encrypted
     */
    private InMemoHedwigMessage(byte[] message, CharSequence sender,
    ...
    private InMemoHedwigMessage(ASAPCryptoAlgorithms.EncryptedMessagePackage encryptedMessagePackage,
    ...
    public static byte[] serializeMessage(byte[] content, CharSequence sender, CharSequence recipient)
        ...
        return InMemoHedwigMessage.serializeMessage(content, sender, recipients,
            false, false, null);
    }
    ...
    public static byte[] serializeMessage(byte[] content, CharSequence sender, Set<CharSequence> receiver,
        ...
        if(receiver == null || receiver.isEmpty()) {
            ...
            receiver.add(HedwigMessageI.ANY_RECEIVER);
        }

        if(sender == null) {
            sender = HedwigMessageI.ANONYMOUS;
        }
    ...
    @Override
    public boolean isLaterThan(HedwigMessageI message) throws ASAPException, IOException {
        if(this.encryptedMessagePackage != null) {
            throw new ASAPSecurityException("content could not be encrypted");
        }
    ...
    public static InMemoHedwigMessage parseMessage(byte[] message, List<ASAPHop> hopsList)
        throws IOException, ASAPException {

        return InMemoHedwigMessage.parseMessage(message, hopsList);

    }

    public static InMemoHedwigMessage parseMessage(byte[] message, List<ASAPHop> hopsList, ASAPKeyStore ASAPKeyStore)
        throws IOException, ASAPException {
        ...
        boolean signed = (flags & HedwigMessageI.SIGNED_MASK) != 0;
        boolean encrypted = (flags & HedwigMessageI.ENCRYPTED_MASK) != 0;
        ...
        if (encrypted) {
            ...
            if (!ASAPKeyStore.isOwner(encryptedMessagePackage.getRecipient())) {
                return new InMemoHedwigMessage(encryptedMessagePackage, hopsList);
        ...
        return new InMemoHedwigMessage(snMessage, snSender, snReceivers, creationTime, verified, encrypted, hopsList);
    }

    public boolean isAnonymousSender(CharSequence peerID) {
        return peerID.toString().equalsIgnoreCase(HedwigMessageI.ANONYMOUS);
    }

    public boolean isAnyRecipient(CharSequence peerID) {
        return peerID.toString().equalsIgnoreCase(HedwigMessageI.ANY_RECEIVER);
    }
}
